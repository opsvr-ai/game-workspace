...
6. **测试数据用完必删，绝不碰真实业务数据。**（老板明确要求，2026-09-21）
   在线上做任何验证（注册测试账号、建测试单/客户、跑报账、模拟抢单、发消息等）之前，
   先想好怎么收尾；验证完立刻把造出来的数据删干净，**连关联记录一起删**
   （聊天室、群成员、考勤、CompanionPC、聊天消息……），
   不要让老板/客服在界面上看到「测试注册xxx」这类脏数据。
   真实客户 / 订单 / 报账 / 支取 / 聊天记录**一律只读**：
   除非老板点名要删，绝不删除、不改写。
   只读探测（登录、心跳、GET）不留数据；写操作（注册、建单、报账、支取）必须自己收尾。
