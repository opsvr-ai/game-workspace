...
- **客服端自动更新闪退:** 修复客服端更新检查用字符串相等判断版本（`===`）导致版本号不一致时反复下载安装并退出（闪退）的问题；改为按数字比较版本号，只有服务器版本严格更新时才更新，并把退出时机改为下载完成后，避免固定 1.5 秒打断安装。同时把客服端安装改为按用户安装（不再装到 Program Files 需要管理员权限），静默自动更新无需提权即可完成。
