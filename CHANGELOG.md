...
- **店长能看到「所有客服的派单记录」（老板 2026-09-24 要求）：**
  ...
  **只能看到自己发的那几条，客服发的全看不见**（线上蠢驴电竞实测：店长 hanlei1 发 31 条、
  客服邵泽慧发 63 条，店长那 63 条一条都看不到）。现在桌面面板对店长显示本店、对老板显示全部的
  派单记录（标题改成「客服派单记录（N）」，每行都带 📋发布人），面板右上角还能按客服筛出某一个人的；
  一次最多铺 100 行，再多会提示去「订单管理 → 全部订单」看完整记录。
  同时给管理端的「订单管理」表加了**客服筛选**（店长/老板可见，客服和陪玩看不到）：
  「客服 邵泽慧」这类带岗位的下拉，选一个就只看她派的单。客服自己那边不变，仍是「我发布的订单」。
  改动：`apps/web/src/pages/dispatch/CSDispatchView.tsx`、`apps/web/src/pages/OrdersPage.tsx`。
