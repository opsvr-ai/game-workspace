...
- **店长能看到「所有客服的派单记录」（老板 2026-09-24 要求）：**
  派单工作台底部那块面板原来一律按「发布人 = 自己」过滤，店长站在自己的派单工作台上
  **只能看到自己发的那几条，客服发的全看不见**（线上蠢驴电竞实测：店长 hanlei1 发 31 条、
  客服邵泽慧发 63 条，店长那 63 条一条都看不到）。现在这块面板对店长显示本店、对老板显示全部
  （标题「本店派单记录（N）」/「派单记录 · 全部工作室（N）」，每行都带 📋发布人），面板右上角
  能按派单人筛出某一个人的；一次最多铺 100 行，再多会提示去「订单管理 → 全部订单」看完整记录。
  同时给管理端的「订单管理」表加了**派单人筛选**（店长/老板可见，客服和陪玩看不到）。
  客服自己那边不变，仍是「我发布的订单」。
- **派单人筛选下拉改成按岗位分组（老板 2026-09-24 追问「筛选里怎么冒出来个徐泽宁」）：**
  下拉原来是把「订单里出现过的所有发布人」平铺出来、名字前只加一个岗位前缀，于是蠢驴电竞的陪玩
  **徐泽宁**混在一串客服里，看着像数据出错。查清了原因：陪玩端的「首单 / 续单 / 复购」是**陪玩自己
  发起**的，而 `POST /orders` 一律把发布人记成当前登录账号（`orders.controller.ts` 里
  `csUserId: req.user.id`），所以陪玩自己开的那条复购单（2026-09-14、35 元、全站仅此 1 条）
  发布人就是他本人——**不是脏数据，也不该藏掉**。现在下拉分成「客服 / 店长-老板 / 陪玩自己开的单」
  三组，一眼看得出这个人是**以什么身份**出现在这里的。改动：`apps/web/src/pages/dispatch/CSDispatchView.tsx`、
...
- **修掉「自动更新把陪玩电脑装成半残」的两个根因（老板 2026-09-23 报：陈佳祺双击桌面图标没反应）：**
  ...
  现象是**字节数一个不差、内容全是错的**。同一台机器连下两次，md5 各不相同、解压报 `invalid data`。
  已实测（修复前后各下一次整包、与服务器上的文件对 md5）确认并修掉：现在每块单独分配内存。
  这条链路上每台客户端都受影响：包烂了，老看门狗照样「装完就报成功」，于是安装目录里出现 0 字节 dll、
  中文名 exe 变成 `????????.exe`（GBK 名的 zip 被按 UTF-8 解），客户端从此谁也点不开。
  ② **看门狗不再「边下边盖」**：解压到旁边的临时目录 → 校验（`resources/app.asar` + 客户端 exe 都在且够大）
  → 整个目录换过去（旧目录留 `.bak-<时间>`，可整目录回滚），换完等客户端自报健康
  （`C:\ProgramData\chunlv\client-healthy.json`），等不到就回滚 + 把这个版本在本机拉黑，不再反复重装同一个坏版本。

- **看门狗 2026-09-23.3（`apps/watchdog-service`，已上线，md5 `ae96a2ceb5fbd78973c75a2e7dda4fac`）：**
  除上面的原子更新外还加了：整包自愈（找不到客户端 / 刚拉起就死 / 活着但一直不自报健康 → 用本机留的包或云端整包重装，
  换完重建快捷方式）；多目录不再拉错（`.chunlv-new-*` / `.bak-*` / `.chunlv-broken-*` 一律不当客户端目录）；
  桌面快捷方式按系统权限给所有用户重建；自愈时先把 URL 下载成本地文件再解压（2026-09-23 实测老写法报
  `open zip: open http://...: The filename, directory name, or volume label syntax is incorrect.`，等于自愈没做）；
  下载用 Content-Length 兜住半截包。服务启动 / 回滚 / 自愈都会把现场回传云端。8 个单测 + 本机端到端演练
  （坏包回滚 + 拉黑、把 exe 改成乱码名后自愈成功）全过。

- **陪玩端 1.0.20260930：** 启动后每分钟写一次健康标记（版本 / exe 路径 / 时间），看门狗据此判断
  「这次更新到底跑起来没有」；被本机拉黑的版本不再下载（否则回滚到旧版后每 30 分钟又把自己更新坏一次）。
  客户端自己不再跑 NSIS 安装器（老路会删目录、杀进程），更新包一律交给看门狗原子换目录。
  实测：本机装完 1.0.20260930 后健康标记正常写入，看门狗日志出现 `client health marker seen — update rollback is armed`。

- **一键修复脚本 `scripts/repair-companion.ps1` + `scripts/repair-companion.bat`（给「已经点不开」的机器用）：**
  管理员跑一次 `http://1.117.229.36:3001/uploads/repair-companion.bat`：回传现场 → 下载整包（限速接口失败自动换直链）
  → 校验（.NET UTF-8 / Expand-Archive / tar 三种解压方式依次试，任一解出完整客户端才算过；旧目录换不走就中止，本机一个文件都不动）
  → 整目录换新（旧目录留 `.broken-<时间>` 不删）→ 停用其它目录里的旧客户端（含乱码名的大 exe）→ 装最新看门狗
  → 重建桌面图标 → 拉起客户端 → 再回传一次现场。`-DiagOnly` 只回传现场、不改任何东西。
  本机按「和陈佳祺同样的故障状态」真实跑通（下载 128MB → 解压 → 换目录 → 装服务 → 修快捷方式 → 客户端起来）。

- **服务端诊断接口 `POST /api/agent/diag-report`（`x-onboard-token` 头）：** 看门狗与一键修复脚本把现场
  落到 `onboard-reports/diag/<主机名>-<时间>-<来源>.log`（不在 `/uploads` 下，公网下不到），
  「客户端出问题人不在现场」时可以直接读。2026-09-23 靠它看清了多目录 / 乱码名 / 半截安装这几件事。

- **杀进程加了「本店黑名单是否生效」开关（老板 2026-09-23 要的）：** 以前只有老板那个全站总开关
  （`blacklist.auto_kill`，一份值管全站），「进程黑名单」页顶部现在多一个**店长自己拨**的开关，
  管**本店**的黑名单要不要真的动手 —— 关掉就只记录、只上报，本店陪玩一个进程都不会被结束，
  一键止血，别家店不受影响（各店开关互不影响，已加回归测试锁住）。
  两个开关是**两道闸，都开才杀进程**：老板的全站总开关仍是总闸（默认关，脚本 / 自动化不许改），
  本店开关默认「生效」，等于「没拨过的店继续跟着老板走」，行为跟以前完全一样。
  开关一拨**当场重推名单**：老板拨全站总开关 → 所有店重推；店长拨本店开关 → 只重推本店；
  「恢复用老板的默认」把本店开关恢复回去时也照样重推（不然客户端手里还留着旧名单）；
  关掉时下发的是**空名单**，连没升级的老客户端也会立刻停下。
  页面顶部还会直接写清「当前实际：会 / 不会结束名单里的进程」以及是哪道闸关着，
  不用自己去猜为什么没生效。REST 兜底拉名单（`GET /api/processes/blacklist/my-rules`）同样受这两道闸约束，
...
- **抢单池 / 派单工作台左侧「人员」列表把名字挤成「陈…」，现在名字完整显示（老板 2026-09-24 报）：**
  实测（1280×720、线上 29 个人员）：246px 的「人员」列里，一行塞了段位徽章（`🐎 中等马` 50px）＋名字＋状态（`● 空闲` 36px）＋聊天按钮（22px），
  中间只剩 **25px** 给名字 —— 三个汉字要 39px，所以「陈佳祺」被截成「陈…」；陪玩端更狠（名字不缩、整行溢出被卡片裁掉，连省略号都没有）。
  现在第 1 行只放「段位 + 名字 + 💬」，状态点＋状态字挪到第 2 行（那里本来就有角色 / 工作室 / 打单中信息），
  名字独占第 1 行剩余宽度（陪玩端 62px、派单工作台 66px），另加省略号兜底 + 悬停显示全名。
  **列宽保持 246px 不变**，只是把挤在一行里的内容分到两行。改后实测两页各 29 个人员：截断 0 个、卡片无横向滚动。
  改动：`apps/web/src/pages/OrderPoolPage.tsx`、`apps/web/src/pages/dispatch/CSDispatchView.tsx`。
- **修掉「修复脚本跑到一半被关掉 -> 看门狗留在停止状态 -> 机器看起来又坏了」（2026-09-24 秦伟杰那台实测到）：**
  他那台第一次双击修复脚本、跑到一半窗口被关掉，现场回传里 `SystemHelper status=Stopped` --
  修复流程为了换目录必须先把看门狗停掉，而「停掉」之后**没有任何人负责收尾**：窗口一关，
  看门狗就一直停着，客户端再也没人拉，一台已经「修了一半」的机器看起来就是「又无缘无故坏了」，
  一直到 6 分钟后第二次跑完才恢复。现在停看门狗之后立刻装一层保险：写一个标记文件
  （`C:\ProgramData\chunlv\repair-active.txt`）+ 起一个盯着它的小进程（`chunlv-watchdog-guard.ps1`），
  标记一消失、或最多等 45 分钟，就确认看门狗在跑、不在就把它拉起来；正常跑完（以及四条中止路径）
  都会主动 `Clear-Guard` 让这个进程十几秒内自己退掉。**这个保险是纯保险**：看门狗已经在跑时它什么都不做，
  连自己的脚本文件都会删掉（本机已实测：会等待、标记消失后自己退出、没误动正在运行的服务）。
  顺带把秦伟杰那台的现场证据记进 `COMPANION_PC_REGISTRY.md`：坏的是
  `C:\Program Files\陪玩管理\陪玩管理.exe` 只剩 97,294,117 字节且没有版本号（Windows 报
  「不是有效的 Win32 应用程序」，双击必然没反应），老目录「蠢驴电竞」里那份 177,101,312 字节 / `1.0.20260865`
  倒是好的 -- 正是「更新时写坏」留下的半截文件；老看门狗从 09-23 03:22 起每 4~5 秒空转拉一次，拉不起来也不会自愈。
...
- **一键修复脚本「换新后比对 exe 大小」量错了时机，改名方式下必然误判并回滚（`scripts/repair-companion.ps1`）：**
  ...
  改名方式下 `Move-Item` 已经把临时目录里那份 exe 搬进安装目录，临时目录里当然量不到，
  校验于是永远判「大小不对（177101312 / 应为 0）」，把刚铺好的新客户端整份删掉、把旧目录放回去：
  下载、解压、旧目录改名、铺新文件全部成功，最后一步自己整体回滚，陪玩白等一场
  （2026-09-24 修马凝初那台实测到：「换新完成」前面那行正是这个误判）。
  现在期望大小在搬 / 拷**之前**量好；同一台机复跑一次完整走通
  （`1.0.20260930` + 看门狗 `2026-09-23.3`，0 字节文件 = 0，客户端进程只在登录会话里那一份）。
  铺好再比对字节数这道校验本身保留 —— 它挡的正是这次事故里「半新半旧」的安装目录。
  顺带说明：覆盖方式（`-ForceOverwrite` / 目录改不了名的机器）不受这个 bug 影响，
  那条路用的是 `Copy-Item`，临时目录还在，所以 2026-09-24 修徐建业那台是好的。


- **一键修复脚本在「会话 0」里跑时不再多开一份看不见的客户端（`scripts/repair-companion.ps1`）：**
  脚本被远程 / 服务方式（SYSTEM、会话 0）调用时，最后一步 `Start-Process` 会在 SYSTEM 账户下开一份
  没有登录态、陪玩也看不到的客户端，白占内存还容易和真正那份打架。现在先看自己跑在哪个会话：
  在登录会话里才自己拉起，在会话 0 就等看门狗用 `CreateProcessAsUser` 把客户端拉进当前登录桌面。
  2026-09-23 远程修机时实测「每台多出 3~4 个会话 0 进程」，改完复修 4 台确认只剩登录会话里那一份。

- **一键修复脚本「旧目录换不走」的根因：动作顺序反了（`scripts/repair-companion.ps1`）：**
  第 1 步原本先杀客户端、再停看门狗，而看门狗盯着客户端 PID —— 那几秒里它立刻把客户端拉回来，
  安装目录一直被占着，第 4 步「旧目录改名」必然失败。2026-09-24 远程修 3 台时全部卡在这一步
  （脚本按设计中止、本机文件一个没动，但机器仍然是坏的，白跑一趟）。现在改成先 `sc stop SystemHelper`
  等它停稳、再杀客户端；找进程也改成「按 exe 路径找」而不是只认那两个进程名（名字被解压搞成
  `????????.exe` 的客户端按名字根本找不到），杀不干净就重试 4 轮；改名失败时把系统原话写进日志
  （原来被 `-ErrorAction SilentlyContinue` 吞掉，只剩一句「换不走」，查不出是谁占着）。
  顺带修掉诊断回传里对「没有目标的快捷方式」调用 `Test-Path` 报的错（日志一片红但不影响修复）。

- **一键修复脚本：整目录改不了名的机器也能修（`scripts/repair-companion.ps1` + `-ForceOverwrite`）：**
  2026-09-24 远程修 3 台机时遇到一种新情况：**整个目录**改不了名（一律「访问被拒绝」），
  但目录里的文件读 / 写 / 改名 / 删都正常，客户端 exe 杀掉后也能写 —— 于是「目录改名 → 新目录顶上」
  这条主路径在这 3 台上永远走不通（反复改名还会让杀毒软件盯得更紧）。现在：改名失败时不再直接放弃，
  而是退一步「**先把旧目录整份复制留底（同 `.broken-<时间>`）→ 再用新文件覆盖着铺进去**」，
  覆盖前先确认客户端 exe 写得动（写不动仍然中止、本机一个文件都不动），铺完比对 exe 字节数与包里一致
  才算成功，不一致就从留底复制回去。改名本身也从 1 次改成 4 次（每次隔 5 秒），等 exe 松开最多等 90 秒。
  新增 `-ForceOverwrite` 开关：知道这台机器目录改不了名时，直接跳过改名走覆盖。
  3 台实测修好（`1.0.20260930` + 看门狗 `2026-09-23.3`，目录里 0 字节文件 = 0）。

- **客服端桌面图标变白 /「无法在此电脑上运行」根因修复（老板 2026-09-22 报：邵泽慧的电脑图标变白、点开提示无法在此电脑上运行）：**
  图标变白只有一个含义：快捷方式指向的 exe 已经不在了（客户端改名「蠢驴电竞客服端 → 客服管理」、
  更新后换目录都会造成这个）。陪玩端早就有「每次启动校正自己的桌面快捷方式 + 清理失效图标」这段自愈
  逻辑，**客服端一直没抄过来**，所以客服机改名/更新后图标就烂在那里，只能人工去删。
  现在客服端启动时也做同样的事（客服端 main.js 新增 ensureDesktopShortcut()，仅打包版执行）：
  ① 把当前用户桌面的「客服管理」快捷方式指到本机正在运行的 exe；
  ② 扫描「当前用户桌面 + 公共桌面」，清掉目标已不存在的同名白图标、指向自己的重复图标，
  以及改名前的「蠢驴电竞 / chunlv」旧图标（公共桌面同名那条正是最常见的白图标来源，所以判重按全路径而不是文件名）。
  顺手修掉一个会让排查白费劲的小坑：公共桌面目录不存在时 Get-ChildItem 报错会让 powershell 以退出码 1 收尾，
  日志里天天躺着一条假的「修复失败」告警，现在先 Test-Path 过滤。

- **「广播单发了，所有人都没弹窗」根因修复 + 新单提醒不再漏（老板 2026-09-22 报：邵泽慧刚发一个广播单，大家都说没弹窗）：**
  先现场复现：用一个测试陪玩 socket 连线上服务端，再发一张真广播单 —— 服务端**确实**把 `order:urgent`
  推给了「状态=空闲」的人（新增日志 `SEND order:urgent (broadcast) {"matched":1,"connected":1}`），
  房间投递本身没问题。真正的问题是**弹窗只存在于窗口里那张卡片上**：陪玩在打游戏 / 缩到托盘时，
  既看不到卡片也**听不到任何声音**（`onOrderUrgent` 以前不播提示音、也不发系统通知），弹窗 20 秒后自动消失，
  一单就这么错过。本次四层兜底，缺一不可：
  ① 网页端收到广播/指定单**立刻播提示音 + 发 Windows 系统通知**（`AppLayout.onOrderUrgent`），
  窗口在后台、最小化也能被叫到；② 陪玩端主进程那条 WebSocket 以前收到 `order:urgent` **直接丢掉**，
  现在窗口不在最前面时**自己画一个置顶小窗**（沿用群聊广播的样式：鼠标穿透、不抢焦点），
  窗口就在眼前时只**闪任务栏**，不打扰正在打游戏的人；③ 弹窗停留时长统一由服务端下发
  （`_popupSeconds` ← 设置里的 `pool.popup_seconds`，默认 20 秒），网页卡片与置顶小窗用同一个数，
  不再「一处可配、另一处写死」；④ 广播推送补齐日志：**命中几人 / 真正在线几人 / 谁不在线**，
  命中 0 人时告警——以后问「为什么没人收到」直接看日志，不用猜。
  陪玩端改动随客户端 **1.0.20260926** 下发（接单中不打断，客户端自己挑空闲更新）。

- **管理端「下载图片包」100% 失败（老板 2026-09-22 报：陪玩上传截图后，管理端点下载图片包提示失败）：**
  根因是服务端**用系统 `zip` 命令打包**，而线上服务器**根本没装 zip**（`which zip` 无此命令，
  只有 `unzip`）——`execFile('zip', …)` 直接 `spawn zip ENOENT`，所以这个功能从上线起就没成功过一次。
  改成**纯 Node 打 zip**（新增 `apps/server/src/common/zip.ts`：零依赖、store 方式不压缩，
  战绩图本来已压过一轮，再 deflate 没收益；带 CRC32、UTF-8 文件名标志，Windows 资源管理器 / 7-Zip / macOS
  都能正常解压），不再依赖服务器上装没装 zip。
  顺带修掉三个同源毛病：① 缺图时 `copyFileSync` 抛 ENOENT 会让整个下载失败，现在**少哪张跳过哪张并记日志**，
  全缺才提示「请让陪玩重新上传」；② 以前把 PNG 也强行改名成 `.jpg`，现在**扩展名跟真实文件走**；
  ③ 业务提示（如「记录不存在」）不再被套成一层看不懂的「打包失败」。
  管理端（`apps/web/src/pages/BattleScreenshotReviewPage.tsx`）同步改：以前无论什么原因都只弹一句
  「下载失败」，现在**直接把服务端给的原因显示出来**；中文文件名按 `filename*=UTF-8''` 解析，
  存下来的文件名不再变成「??_2026-09-22.zip」；文件名里的日期也改按**北京时间**取
  （服务器跑 UTC，半夜 0 点后上传的图以前会被标成前一天，`chinaDateOf`）。
  另外给线上服务器**装上 zip**，让「一键构建发布」那条老链路（`agent.service.ts` 里的 `zip -r`）不再踩同一个坑。

- **陪玩端「动不动就不自动登录」（老板 2026-09-22 报）：** 两个根因，都只在客户端重启后才暴露：
  ① 网页端把 accessToken 存在 sessionStorage 里 —— 客户端**每次重启**（崩溃自愈、看门狗拉起、更新、开机）
  它一定是空的；唯一能补救的是「记住我」保存的账号密码，可它**默认没勾**，而且**不勾就登录一次会把之前
  保存的账号密码擦掉**（`credentials:save` 收到空账号即清空）。于是没勾过的人每次重启都得手打一遍密码。
  现在启动时先读主进程里那份**还在有效期内的令牌**（`config.json` 的 `token` / `refreshToken`，7 天有效，
  看门狗换令牌也会写回）塞回网页端，直接恢复登录、全程不需要密码
  （新增 `apps/web/src/utils/sessionRestore.ts`，「退出账号」仍会清掉它，所以退出过的机器照样显示登录页）。
  ② `fetchUser()` 以前**任何异常都清登录态**：断网、25 秒超时、服务端重启的 5xx 全算，办公室那条网一抖
  所有人被踢回登录页，而且连 7 天有效的 refreshToken 一起清掉（`stores/authStore.ts`）。现在只有服务端
  明确 401/403（axios 拦截器已经试过用 refreshToken 续期）才清。线上证据：9/21 一天，光老板那个出口 IP
  就登录了 **95 次**。另外「记住我」默认改成勾上（一人一台机器的接单工具，登录一次就该一直记住）。
  登录态确认失效时，会**同时清掉主进程那份令牌**（`clearStoredSession`），否则登录页会一遍遍把旧令牌读回来，
  变成「登录页 → 恢复 → 401 → 又回登录页」的自刷新死循环。
  已部署：网页端 `v765 → v766`（只动网页端，不用发客户端 exe，客户端 5 分钟内自己热更）。
...
- **「工资规则」一页管两个岗位，「店长设置」页删除（老板 2026-09-22「一样的功能全部放在一起，要不然乱七八糟」）：**
  ...
  店长 / 客服**一岗一行**，横向是 基本工资 / 月休天数 / 迟到扣款 / 缺勤扣款；客服那行多两列
  「早退扣款 / 全勤奖」（目前只有客服参与结算，所以店长那两格画「—」）；同一页往下是原有的「考勤登记」与「工资表」。
  「店长设置」页与菜单项已删除（`/admin/store-manager-settings` 留一行兼容跳转到工资规则，跟当初删「利润分成」页一个做法，
  避免谁存过旧书签点进去白屏）；「客服设置」只留客服提成与桥接达标规则，并加了一句指向工资规则的说明 + 一个按钮。
  客服底薪在系统配置里的那份兜底值（`commission.cs_base_salary_yuan`）仍与 `PayrollConfig` 一起写，两边永远一个数。
  **陪玩故意不在这张表里**：陪玩收入 100% 来自提成（在「分账规则」里按流水档位设），系统里没有陪玩底薪，
  摆一行出来只会让人以为填了有用。店长账号看自己那一行是只读的并写明「店长工资只有老板能改」
  ——服务端本来就拒绝店长改自己的工资，原来那页点了保存只会弹一句错误。
  **顺带修掉一个「填了不生效」的死设置**：`commission.cs_early_leave_deduction_yuan`（客服早退扣款）在结算里
  是真的会扣的（`commission.service.ts` 数 `StaffAttendance.status === 'EARLY_LEAVE'`），但「考勤登记」的状态下拉
  只有 正常 / 迟到 / 缺勤，**全系统没有任何地方能登记「早退」** → 这个数一直是摆设。现在考勤登记加了「早退」选项。
  左侧栏那个比例小字也顺手挪了：原「店长设置」项后面挂着的店长比例，现在挂在「工资规则」上（客服仍在「客服设置」）。
  上线实测（读原值 → 原样写回 → 再读对比，跑完已还原）：店长给自己定工资 → **403 被拒**；
  店长改客服工资 → **201 成功**；造出来的 `PayrollConfig` 测试行已删除，表恢复 0 行、`PayrollRecord` 未新增。
  网页已部署 **v774**。
  （已知问题，未动：`PayrollConfig` 是按 `role` 全站唯一，**店长改客服/店长工资会影响所有店**，
  还没做按店隔离 —— 与「店跟店相互独立」的口径有出入，等老板点头再改。）
...
- **同一批设置散在好几页，已合并（老板 2026-09-22 报：「分成在财务管理也有、设置也有」）：**
  ...
  三处都能改 —— 现在**只剩「设置 → 系统配置 → 利润分成（分账规则）」一页**能改（含 ≤100% 校验、
  工作室自动算）；独立的「利润分成」页已删除（老书签 `/admin/profit-split` 自动跳到这一页），
  「客服设置」里的客服分成比例改成只读 + 「去利润分成修改」；
  ② **桥接机密 / 绝密** 原先「财务中心」能改单价、「派单与提成」能改首单返款 —— 现在两组都搬到
  「利润分成（分账规则）」页最下面的「桥接工作室」块（机密单价 / 绝密净价 / 首单返款，库里的分值自动换算成元），
  「财务中心」的单价改成只读 + 跳转；
  ③ **每日新客户名额（上等马 / 中等马 / 下等马）** 原先「流水与价格」「派单与提成」「评分与名额」三处都能改 ——
  现在只在「评分与名额」里改，另外两处已移除。
  顺手删掉一个**填了不参与任何计算**的死配置 `dispatch.studio_share_percent`（派单页原来的「工作室分成比例」），
  并把「系统配置」做成支持直达链接（`/admin/settings?tab=payment`），方便别处一句话跳过去。
  文档同步：「用户手册」「架构文档」里指向「利润分成」页的说法全部改成新位置。

- **陪玩端客户端 1.0.20260925 发布（老板 2026-09-22「所有没发布的全部发客户端 exe」）：** 把仓库里已经改好、
  但一直没进安装包的客户端改动一次性发出去：① **接单中不执行推送更新**（`electron/updater.ts`：
  下载前、下载后各等一次空闲，最多 30 分钟；等不到就把已下好的包留在本地、这一轮不重启）——
  以前后台「推送更新」会直接把正在跑的单子掐断；② 装机脚本改用真正被加载的 `build/installer.nsh`
  （安装目录 `C:\Program Files\陪玩管理`）；③ 移除陪玩端不会被加载的前端渲染层；④ 合并被抄两遍的工具函数。
  同时补强发布链路（`scripts/_publish_client.py`）：**装机包与更新包一起发**（以前只换更新包，
  新电脑装出来还是旧版）、**自动更新地址写成限速接口 `/api/agent/download/latest`**
  （脚本原来会把它改回全速直链，等于把限速修复覆盖掉）、发布前先比对线上版本号，
  **不大于线上就直接中止**（客户端只在服务端版本更高时才升级，版本号漏改会「上传成功但一台都不升」）。
  线上自查：更新包 md5 `b32984044c40…`、装机包 md5 `91a857d1318a…`（与本地一致）、
  `/api/agent/download/latest` 限速 ~744KB/s、`/api/agent/download/exe` 完整 91861787 字节、
  `agent.latest_version = 1.0.20260925`。**没有做强制推送**，接单中的机器自己 5 分钟后轮询到、空闲时才交接重启。
