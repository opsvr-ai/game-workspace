...
- **续单换搭档时旧搭档无感知/状态不释放:** 主陪续单换搭档后，之前只结束旧会话并通知新搭档，被换掉的旧陪玩（如王昊）既不知道“这一段打完了”，也不知道本段计入了多少流水，状态还会卡在接单。现在续单时会给被换掉的旧陪玩推送「这一段服务已结束 + 本段计入流水 ¥xx」提醒，并把其状态自动释放回空闲（若没有其他进行中服务）。
