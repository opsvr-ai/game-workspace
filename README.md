...
- **店长可以看到所有客服的派单记录（2026-09-24）:** 派单工作台底部那块面板以前一律按「发布人 = 自己」过滤，店长站在自己页面上只能看到自己发的单，客服发的全看不见（线上蠢驴电竞：店长 31 条、客服邵泽慧 63 条，那 63 条店长一条看不到）。现在店长看本店、老板看全部（标题「本店派单记录（N）」/「派单记录 · 全部工作室（N）」），每行带 📋发布人，右上角可按派单人筛一个人；一次最多铺 100 行，更多去「订单管理 → 全部订单」，那里也加了「派单人筛选」（店长/老板可见）。**下拉按「客服 / 店长-老板 / 陪玩自己开的单」分组**——老板追问「筛选里怎么冒出来个徐泽宁」，查清是陪玩端的「首单/续单/复购」由陪玩自己发起、发布人就是他本人（全站仅此 1 条 35 元复购单），分组标题把身份写明，不会看着像脏数据。客服自己那边不变，仍是「我发布的订单」。
- **人员列表名字不再被截断（2026-09-24）:** 老板报「抢单池左侧人员列表名字显示不全」。量下来是 246px 的一行里塞了段位徽章（50px）+ 状态（36px）+ 聊天按钮（22px），只剩 25px 给名字，三个汉字（39px）必然截成「陈…」（陪玩端名字不缩、整行溢出被卡片裁掉，更难看）。现在第 1 行只放「段位 + 名字 + 💬」，状态挪到第 2 行（那里本来就有角色/工作室，打单中还有游戏名），名字独占剩余宽度（62–66px），长名字才用省略号收尾、悬停显示全名。**列宽 246px 没动**，两页各 29 个人员实测零截断、无横向滚动。
- **一键修复脚本顺手把机器配成「可远程维护」（2026-09-24，为秦伟杰那台加的）:** 老板报「秦伟杰也打不开、图标都变了」（他那台地址 `192.168.0.127`），一查发现那台机器**从来没跑过我们的装机脚本** —— 本机没有 `chunlvops` 账号、也没开远程管理通道，我们连不进去、也看不到现场，只能干等人在电脑跟前。现在 `repair-companion` 多了一步：修之前先把这台机配成标准状态（建 `chunlvops` + 开远程管理通道 + 把主机名/IP/MAC/账号回传云端），以后同类故障直接远程看。**已经配好的机器不动密码**（回传 `accountEvent=kept`），免得把本来能用的那把换掉、回传又失败就彻底进不去。装机入口的两处同类隐患一并修掉：装机 bat 用 `net session` 判管理员在「Server 服务停着」的新机器上会无限弹授权窗口（换成 `fltmc`），回传的局域网地址改成按「到云服务器的实际出口网卡」取（免得 VMware / VirtualBox 虚拟网卡抢答、把错地址回传上来）。
- **「双击桌面图标没反应」的根因修完了（2026-09-23）:** 老板报陈佳祺那台陪玩电脑点了图标没反应。查出来是两个叠一起的根因：**服务端下发更新包时会塞进烂字节**（`common/throttled-file.ts` 复用同一个 64KB buffer，遇到背压时没发出去的那块被下一轮读覆盖 —— 字节数一个不差、内容全错；同一台机器连下两次 md5 都不一样），包烂了以后**老看门狗照样报「更新成功」**，于是目录里留下 0 字节 dll、中文名 exe 变成 `????????.exe`，客户端再也点不开。现在：下发每块单独分配内存（已实测整包 md5 与服务器一致）；看门狗（`2026-09-23.3`）改成「解压到旁边的临时目录 → 校验 → 整个目录换过去 → 等客户端自报健康 → 等不到就回滚 + 拉黑这个版本」，起不来还会整包自愈重装；陪玩端 `1.0.20260930` 每分钟写健康标记、被拉黑的版本不再下载、更新包不再自己跑安装器。已经点不开的机器（比如陈佳祺）用一键修复脚本：管理员跑 `http://1.117.229.36:3001/uploads/repair-companion.bat`，九件事一把做完（**顺手把这台机配成可远程维护** → 现场回传 → 下包 → 校验 → 换目录 → 停用旧目录客户端 → 装看门狗 → 重建图标 → 拉起客户端）。本机按同样故障状态跑通，看门狗的自愈路径也单独演练过。现场诊断回传见 `POST /api/agent/diag-report`。
- **杀进程只剩「店长自己那一个开关」（2026-09-24，老板要的）:** 老板明确说「不需要总闸，只需要店长自己定自己的俱乐部或者工作室是否生效就可以」，所以以前那个全站总开关 `blacklist.auto_kill` **整条去掉了**，现在只有「进程黑名单」页顶部的**本店黑名单是否生效**（`blacklist.enabled`，店长自己拨，只影响本店，各店互不影响）。**默认「不生效」**——跟去掉总闸之前的线上现状一致（名单只记录、一条都没杀过），店长想真的动手自己拨开即可，一拨就当场重推名单（关掉时下发**空名单**，老客户端也会立刻停，不用等升级）；「恢复本店默认」也照样重推。页面直接写着「当前实际：会 / 不会结束名单里的进程」。判定收口在 `apps/server/src/common/blacklist-switch.ts`，REST 兜底拉名单同样受约束（WebSocket 断了也绕不过去）。老板账号那一张卡片改成纯说明，不再需要他拨任何开关。
...
- **重复的设置项彻底删干净（2026-09-22）:** 上一版把重复的分成项改成「只读 + 去XX修改」，老板还是嫌绕；现在「客服设置 → 客服提成」里那两行只读副本（线下提成比例 / 线上每单提成）**整行删除**，这两个数只在「设置 → 系统配置 → 利润分成（分账规则）」里改；「财务中心」的桥接单价卡片按老板要求保留为只读 + 一个跳转按钮。顺带复查：桥接单价/返款与四个人分成的键，现在全站只有分账规则页一处能写。
...
- **新单弹窗更合理:** 弹窗秒数可配（`pool.popup_seconds`，线上 20 秒，原来写死 15）；**空闲 + 娱乐中一律弹**，只有「正在给别人打单」默认不打扰、本人可在陪玩端设置里打开「打单中也接收新单弹窗」（`/api/companions/me/notify-prefs`）；广播急单对自家工作室立即可见，不再排段位等 60/120 秒。
...
- **派单方式语义分开:** 「广播」= 本店所有在线、状态为「空闲 / 娱乐中」的陪玩右下角弹窗抢单（订单同时进池）；「入池」= 只进抢单池、不弹窗；「指定」= 只弹给被指定的陪玩。修掉了「入池 + 立即」被当成急单弹给所有人、以及「指定 + 立即」把指定单泄漏给全店的问题。
- **发布订单「广播」修复:** 派单方式选「广播」原先会被服务端校验拦下并报错，现在可以正常发布；订单照常进池，同时立刻推送给工作室在线、状态为「空闲 / 娱乐中」的陪玩。
