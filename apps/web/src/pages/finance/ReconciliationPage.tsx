    ...
    <div>
      ...
      <Card size="small" title={`${day.format('YYYY-MM-DD')} 营业日对账`}>
        ...
        >
          ...
          <Table.Column title="应收（订单）" dataIndex="expectedYuan" render={(v: number) => `¥${Number(v || 0).toFixed(1)}`} />
          <Table.Column title="到账（员工码）" dataIndex="actualYuan" render={(v: number) => `¥${Number(v || 0).toFixed(1)}`} />
          <Table.Column
            title="差额"
            dataIndex="diffYuan"
            render={(v: number) => {
