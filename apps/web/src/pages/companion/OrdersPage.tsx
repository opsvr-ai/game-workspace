          ...
          <>
            ...
            {r.status === 'GRABBED' && !r.contactStatus && (
              <>
                ...
                >
                  <Button size="small" icon={React.createElement(UploadOutlined)}>
                    上传截图
                  </Button>
                </Upload>
                <Button
                ...
                >
                  <span style={{ cursor: 'pointer', color: '#ff4d4f', fontSize: 14, marginRight: 8 }}>
                    📎 已添加未同意
                  </span>
                </Upload>
              </>
            ...
            {r.status === 'GRABBED' && r.contactStatus === 'added' && (
              <>
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await http.post(`/orders/${r.id}/confirm`);
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await http.post(`/orders/${r.id}/republish`);
                      message.success('已发布到抢单池');
                      fetch();
                    } catch (e: any) {
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (dt)
                      http
                        .put(`/orders/${r.id}/contact`, { scheduledAt: new Date(dt).toISOString() })
                        .then(() => {
                          message.success('已预约');
                          fetch();
                        })
            ...
            {r.status === 'CONFIRMED' && (
              <>
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await http.post(`/orders/${r.id}/renew`);
                      message.success('已续单');
                      fetch();
                    } catch (e: any) {
                      message.error(e?.response?.data?.message || '续单失败');
                    }
                  }}
                >
                  续单
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (amt && !isNaN(+amt))
                      http
                        .put(`/orders/${r.id}/amount`, { amount: +amt })
                        .then(() => {
                          message.success('已改价');
                          fetch();
                        })
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          ...
          >
            <Space size="large">
              ...
              <span>
                ...
                <Text strong style={{ fontSize: 16, color: '#FF4757' }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 4 }}>
              <Upload beforeUpload={handleUpload} maxCount={1} listType="picture-card">
