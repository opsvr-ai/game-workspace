...
export default function BridgePage() {
  ...
  const pendingColumns = [
    { title: '申请工作室', key: 'studio', render: (_: unknown, r: BridgeInfo) => {
      ...
      return <Text>{otherName} {isIncoming ? <Tag color="blue">待我处理</Tag> : <Tag>已发出</Tag>}</Text>;
    }},
    { title: '时间', dataIndex: 'createdAt', render: (v: string) => new Date(v).toLocaleString() },
    { title: '操作', key: 'actions', render: (_: unknown, r: BridgeInfo) => {
      const isIncoming = r.studioBId === user?.studioId || (r.studioAId === user?.studioId && r.proposedBy !== user?.id);
      if (!isIncoming) return <Tag>等待对方</Tag>;
      return (
        <Space>
          <Button size="small" type="primary" icon={<CheckOutlined />} onClick={() => handleRespond(r.id, true)}>同意</Button>
          <Button size="small" danger icon={<CloseOutlined />} onClick={() => handleRespond(r.id, false)}>拒绝</Button>
        </Space>
      );
    }},
  ];
  ...
  const activeColumns = [
    { title: '桥接工作室', key: 'studio', render: (_: unknown, r: BridgeInfo) => {
      const otherName = r.studioAId === user?.studioId ? r.studioB.name : r.studioA.name;
      return <Text strong><LinkOutlined /> {otherName}</Text>;
    }},
    { title: '开通时间', dataIndex: 'acceptedAt', render: (v: string) => v ? new Date(v).toLocaleString() : '-' },
    { title: '操作', key: 'actions', render: (_: unknown, r: BridgeInfo) => (
      <Button size="small" danger icon={<DisconnectOutlined />} onClick={() => handleRemove(r.id)}>断开</Button>
    )},
  ];
  ...
  const availableStudios = studios.filter((s) => s.id !== user?.studioId && !bridgedIds.has(s.id) && !pendingIds.has(s.id));

  return (
    <Card title="工作室桥接管理">
      <Tabs
        tabBarExtraContent={
          <Button type="primary" icon={<LinkOutlined />} onClick={() => setProposeVisible(true)}>申请桥接</Button>
        }
        items={[
          {
            key: 'active', label: `活跃桥接 (${bridges.active.length})`,
            children: <Table rowKey="id" columns={activeColumns} dataSource={bridges.active} loading={loading} />,
          },
          {
            key: 'pending', label: `待处理 (${bridges.pending.length})`,
            children: <Table rowKey="id" columns={pendingColumns} dataSource={bridges.pending} loading={loading} />,
          },
        ]}
      />
      <Modal title="申请工作室桥接" open={proposeVisible} onOk={handlePropose} onCancel={() => setProposeVisible(false)}>
        <Select
          style={{ width: '100%' }}
          placeholder="选择目标工作室"
