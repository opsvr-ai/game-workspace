...
function loadMsgs(userId: string, orderId: string): ChatMsg[] {
  try { const r = localStorage.getItem(`${STORAGE_PREFIX}-${userId}-${orderId}`); return r ? JSON.parse(r) : []; }
  catch { return []; }
}
      ...
      {partner && (
        <>
                      ...
                      {m.text.split(/(\[img\].*?\[\/img\])/g).map((part: string, i: number) => {
                        if (part.startsWith('[img]') && part.endsWith('[/img]')) {
                          ...
                          return <img key={i} src={url} style={{ maxWidth: 120, maxHeight: 120, borderRadius: 4, display: 'block', cursor: 'pointer' }} alt="emoji" title="右键收藏此表情" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                            onContextMenu={(e2) => { e2.preventDefault(); if (!customEmojis.includes(url)) { syncCustomEmojis([...customEmojis, url]); message.success('已收藏到我的表情'); } else { message.info('已在收藏中'); } }} />;
                        }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: '#F7F7F7', borderTop: '1px solid #E0E0E0', flexShrink: 0 }}>
            ...
            <Input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onPressEnter={send}
            ...
            <div style={{ padding: '8px 10px', background: '#FFF', borderTop: '1px solid #E0E0E0', maxHeight: 240, overflowY: 'auto', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
                {(['😀','😂','🤣','😊','😍','🥰','😘','😜','🤔','😅','😭','🥺','😤','🤬','👍','👎','🙏','💪','🔥','❤️','💔','⭐','🎉','🎂','💰','📱','💻','✅','❌','⚠️','🟢','🔴','🐶','🐱','🦊','🐼','🐨','🐸','🐵','🦁','🐮','🐷','🌹','🌸','🌺','🍎','🍉','🍕','🍔','🎮','⚽','🏀','🚗','✈️','🏠','⏰','📅','💡','🔑','🎵','🌈','☀️','🌙','⛄'] as string[]).map(e => (
                  <span key={e} style={{ fontSize: 22, cursor: 'pointer', padding: '2px 4px', borderRadius: 4 }} onClick={() => { setInput(prev => prev + e); setShowEmoji(false); inputRef.current?.focus(); }}>{e}</span>
                ))}
              </div>
              {/* Custom emojis */}
                  ...
                  {customEmojis.map((url: string, i: number) => (
                    <span key={i} style={{ cursor: 'pointer', padding: 2, borderRadius: 4, position: 'relative' }}
                      ...
                      <span style={{ position: 'absolute', top: -4, right: -4, fontSize: 10, cursor: 'pointer', color: '#FF4757', background: '#FFF', borderRadius: '50%', width: 14, height: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        onClick={(e2) => { e2.stopPropagation(); syncCustomEmojis(customEmojis.filter((_:string,j:number) => j !== i)); }}>×</span>
                    </span>
                  ))}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 4, paddingTop: 8, borderTop: '1px solid #F0F0F0' }}>
                ...
                <Button size="small" onClick={() => { const url = addEmojiUrl.trim(); if (url && !customEmojis.includes(url)) { syncCustomEmojis([...customEmojis, url]); setAddEmojiUrl(''); } }}>收藏</Button>
              </div>
            </div>
          )}
        </>
