...
const SECTIONS = [
  ...
  },
  {
    ...
    key: 'dispatchTypes',
    items: [
      { k: 'POOL', l: '抢单池', c: '#1677ff' },
      { k: 'DIRECT', l: '直接派', c: '#52c41a' },
    ],
  },
  {
    title: '打单时间',
  ...
  },
  {
    ...
    key: 'billingMode',
    items: [
      { k: 'hour', l: '按小时', c: '#1677ff' },
      { k: 'round', l: '按局', c: '#52c41a' },
    ],
  },
  {
    title: '陪陪数量',
              ...
              <Col span={8} key={sec.key}>
                ...
                {sec.items.map(({ k, l, c }) => (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <Tag
                      color={c}
                      style={{ margin: 0, fontSize: 11, width: l.length > 2 ? 44 : 32, textAlign: 'center' }}
                    >
                      {l}
                    </Tag>
                    <Switch
                      size="small"
                      checked={prefs?.[sec.key]?.[k] !== false}
