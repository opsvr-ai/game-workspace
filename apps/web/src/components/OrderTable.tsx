...
const OrderTable: React.FC<Props> = ({ dataSource, loading, renderActions }) => (
  <Table size="small" dataSource={dataSource} rowKey="id" loading={loading}
    ...
    columns={[
      ...
      { title: '微信号', key: 'wechatId', render: (_: any, r: any) => <Text>{r.customFields?.customerWechat || r.customer?.wechatId || '-'}</Text> },
      ...
      { title: '陪玩', key: 'companion', width: 80, render: (_: any, r: any) => r.companion?.user?.username || <Text type="secondary">-</Text> },
      { title: '最近跟进', key: 'followUp', width: 120, render: () => <Tag color="orange" style={{fontSize:10}}>-</Tag> },
      ...
      ...(renderActions ? [{ title: '操作', key: 'actions', width: 280, render: (_: any, r: any) => <Space size={2} wrap>{renderActions(r)}</Space> }] : []),
    ]}
  />
);
