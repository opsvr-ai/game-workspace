...
export const DEFAULT_CONFIGS: Record<string, any> = {
  ...
  // 陪玩处于「空闲/娱乐」等状态时，客户端是否自动结束状态黑名单里的进程，就看这一个开关
  // （老板 2026-09-24 拍板：不要全站总闸，店长自己定自己的俱乐部 / 工作室是否生效）。
  // 关着 = 本店名单只记录、不下发给本店陪玩，进程一个都不会被结束；
 ...
 *  2. **一份值绑住全站**：客户端与网页版本号（推给所有人更新）、
 *     服务器级连接参数（WebSocket 令牌宽限期 / 离线宽限期 / 服务中会话自动清理）。
 *
 * 注：杀进程总开关 `blacklist.auto_kill` 已经**去掉**（老板 2026-09-24：不要总闸，只留店长自己那个
 * 本店开关 `blacklist.enabled`）。所以这个键不在下面的名单里，店长可以自己拨自己店的那一个。
 *
